...
router.post('/addFriend', async (req, res) => {
    ...
    try {
        ...
        // Parse or default to empty array
        const updatedFriendsList = user.friendsList ? JSON.parse(user.friendsList) : [];
        const updateUserList = friend.friendsList ? JSON.parse(friend.friendsList) : [];

        console.log('Friend List of User:', updatedFriendsList);
        console.log('Friend List of Friend:', updateUserList);
        ...
        const updatedReceivedRequests = user.recivedRequests
            ? JSON.parse(user.recivedRequests)
            : [];
        const updatedReceivedRequestsWithoutFriend = updatedReceivedRequests.filter(
            (request) => request !== friendID
        );

        await pool.query('UPDATE users SET recivedRequests = ? WHERE id = ?', [
            JSON.stringify(updatedReceivedRequestsWithoutFriend),
            token,
        ]);
...
router.post('/sendRequest', async (req, res) => {
    ...
    try {
        ...
        // Parse friendsList or default to an empty array
        const friendsList = user.friendsList ? JSON.parse(user.friendsList) : [];
        if (friendsList.includes(token)) {
            return res.status(400).json({ error: 'Friend already exists' });
        }

        // Parse recivedRequests or default to an empty array
        ...
        await pool.query('UPDATE users SET recivedRequests = ? WHERE username = ?', [
            JSON.stringify(updatedFriendsRequest),
            username,
        ]);
